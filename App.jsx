...
function YTPanel({ onConnect, onClose }) {
  ...
  const connect = async () => {
    ...
    try {
      const res  = await fetch(
        `https://www.googleapis.com/youtube/v3/channels?part=statistics&forHandle=TheAuthorRach&key=${key.trim()}`
      );
      const data = await res.json();
      if (data.error) {
        setStatus("error");
        setErrMsg(data.error.message || "API error — check your key and that YouTube Data API v3 is enabled.");
        return;
      }
      const subs = data?.items?.[0]?.statistics?.subscriberCount;
      if (subs) {
        onConnect(parseInt(subs));
        setStatus("ok");
        setTimeout(onClose, 1200);
      } else {
        setStatus("error");
        setErrMsg("Connected but couldn't find the channel. Double-check the API key has no referrer restrictions.");
      }
    } catch (e) {
      setStatus("error");
      setErrMsg("Network error. If you're testing locally this should work fine — referrer restrictions can block it in other environments.");
    }
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000065", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
      onClick={onClose}>
      <div style={{ background: IVORY, borderRadius: "14px", padding: "2rem", width: "100%", maxWidth: "420px", boxShadow: "0 8px 40px #00000030" }}
        onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
          <h3 style={{ margin: 0, fontFamily: "Georgia,serif", fontStyle: "italic", color: WINE, fontSize: "1.1rem", fontWeight: 300 }}>Connect YouTube live</h3>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: "1.2rem" }}>✕</button>
        </div>

        <div style={{ fontSize: "0.72rem", color: "#777", lineHeight: 1.75, marginBottom: "1rem", background: `${CHARCOAL}07`, borderRadius: "8px", padding: "0.75rem" }}>
          <strong style={{ color: CHARCOAL }}>Setup (one time):</strong><br />
          1. Go to <a href="https://console.cloud.google.com" target="_blank" rel="noreferrer" style={{ color: ROSE }}>console.cloud.google.com</a><br />
          2. Create a project → enable <strong>YouTube Data API v3</strong><br />
          3. Create an API key → under key restrictions, set <strong>HTTP referrers</strong> to your Vercel URL (e.g. <code style={{ fontSize: "0.68rem" }}>https://your-site.vercel.app/*</code>)<br />
          4. Paste key below
        </div>
        ...
        <input
          ...
          style={{ width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: `1px solid ${ROSE}55`, fontSize: "0.78rem", boxSizing: "border-box", fontFamily: "monospace", background: "#fff", marginBottom: "0.6rem" }}
        />

        <button onClick={connect} disabled={status === "loading" || !key.trim()}
          style={{ background: status === "ok" ? "#22c55e" : WINE, color: IVORY, border: "none", padding: "0.65rem 1.25rem", borderRadius: "8px", cursor: "pointer", fontSize: "0.75rem", letterSpacing: "0.08em", width: "100%", fontFamily: "inherit", transition: "background 0.2s" }}>
          {status === "loading" ? "Connecting…" : status === "ok" ? "✓ Connected!" : "Connect YouTube →"}
        </button>

        {status === "error" && (
          <div style={{ marginTop: "0.75rem", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "0.65rem 0.85rem", fontSize: "0.7rem", color: "#991b1b", lineHeight: 1.65 }}>
            <strong>Error:</strong> {errMsg}
          </div>
        )}

        <p style={{ fontSize: "0.62rem", color: "#bbb", marginTop: "0.75rem", lineHeight: 1.6 }}>
          Key is stored in your browser session only — never sent anywhere except Google's API.
        </p>
      </div>
    </div>
  );
...
export default function MediaKit() {
  ...
  // Stats state — loaded from /public/stats.json on mount
  ...
  const [ig, setIg] = useState({ followers: 50616, posts: 840, live: false });
  ...
  const [lastIgPull, setLastIgPull] = useState(null);
  ...
  useEffect(() => {
    fetch("/stats.json")
      ...
      .catch(() => setStatsLoaded(true)); // fallback to defaults silently
  }, []);

  // Pull live IG via VidIQ
  const pullIG = async () => {
    setIgLoading(true);
    try {
      const res  = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: "Use VidIQ to fetch Instagram profile @theauthorrach. Return ONLY raw JSON: {\"followers\": number, \"posts\": number}. No markdown, no explanation.",
          messages: [{ role: "user", content: "fetch instagram stats theauthorrach" }],
      ...
      if (tr?.content?.[0]?.text) {
        try { const r = JSON.parse(tr.content[0].text); parsed = r.followerCount ? { followers: r.followerCount, posts: r.postCount } : (r.followers ? r : null); } catch {}
      }
      if (!parsed) {
        const tb = data.content?.find(b => b.type === "text");
        if (tb?.text) { try { const r = JSON.parse(tb.text.replace(/```[\w]*\n?|```/g,"").trim()); parsed = r.followerCount ? { followers: r.followerCount, posts: r.postCount } : r; } catch {} }
      }
      if (parsed?.followers) { setIg(p => ({ ...p, ...parsed, live: true })); setLastIgPull(new Date().toLocaleTimeString([],{ hour:"2-digit", minute:"2-digit" })); }
    } catch {}
    setIgLoading(false);
  };
  ...
  if (!statsLoaded) return (
    <div style={{ minHeight: "100vh", background: WINE, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <span style={{ fontFamily: "Georgia,serif", fontStyle: "italic", color: IVORY, opacity: 0.5, fontSize: "1rem" }}>Loading…</span>
    </div>
  );
  ...
  return (
    <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", background: IVORY, color: CHARCOAL, minHeight: "100vh" }}>
      ...
      {showYT && <YTPanel onConnect={subs => setYt(p => ({ ...p, subscribers: subs, live: true }))} onClose={() => setShowYT(false)} />}
      ...
      <div style={{ background: WINE, padding: "3rem 2.5rem 2.25rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap", marginBottom: "1.6rem" }}>
          {[
            ...
            { p: "Instagram", v: fmt(ig.followers),   s: "followers",                    c: ROSE, live: ig.live },
      ...
      <div style={{ background: CHARCOAL, display: "flex", alignItems: "center", padding: "0 1.5rem", position: "sticky", top: 0, zIndex: 100, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "7px", padding: "0.4rem 0" }}>
          {ig.live && <span style={{ fontSize: "0.58rem", color: "#4ade80", display: "flex", alignItems: "center", gap: 4 }}><Pulse /> IG{lastIgPull ? ` · ${lastIgPull}` : ""}</span>}
          {yt.live && <span style={{ fontSize: "0.58rem", color: "#4ade80", display: "flex", alignItems: "center", gap: 4 }}><Pulse /> YT LIVE</span>}
          <button onClick={pullIG} disabled={igLoading}
            style={{ background: "none", border: `0.5px solid ${GOLD}60`, color: GOLD, padding: "0.22rem 0.6rem", borderRadius: "20px", cursor: igLoading ? "not-allowed" : "pointer", fontSize: "0.58rem", letterSpacing: "0.1em", opacity: igLoading ? 0.5 : 1, fontFamily: "inherit" }}>
            {igLoading ? "…" : "↻ IG"}
          </button>
          <button onClick={() => setShowYT(true)}
            style={{ background: "none", border: `0.5px solid ${IVORY}25`, color: `${IVORY}55`, padding: "0.22rem 0.6rem", borderRadius: "20px", cursor: "pointer", fontSize: "0.58rem", letterSpacing: "0.1em", fontFamily: "inherit" }}>
            ⚙ YT live
          </button>
        </div>
      </div>
      ...
      {tab === "Overview" && (
        <div style={{ padding: "2.5rem" }}>
          <div style={{ background: "#fff", borderRadius: "14px", padding: "2rem", border: `0.5px solid ${ROSE}28`, borderTop: `3px solid ${WINE}`, marginBottom: "2rem" }}>
            ...
            <p style={{ fontFamily: "Georgia,serif", fontStyle: "italic", fontSize: "1.1rem", color: WINE, lineHeight: 1.85, margin: "0 0 1rem" }}>
              "A romance author and creator who lives inside fandom — and who knows exactly where the bodies are buried in publishing."
            </p>
            <p style={{ fontSize: "0.875rem", color: "#555", lineHeight: 1.85, margin: "0 0 0.75rem" }}>
              Rachel D. Oak occupies a genuinely rare dual position: she's a reader with native fandom fluency (her audience reads AO3, demanded Heated Rivalry outside Canada, and will defend her on Reddit) and an author who has seen publishing from the inside — and isn't afraid to say so on camera.
            </p>
            <p style={{ fontSize: "0.875rem", color: "#555", lineHeight: 1.85, margin: 0 }}>
              Short-form: relatable all-caps reader moments. Long-form: industry critique and video essays with cheeky humour and 2am-texting-your-friends energy underneath. Active on every platform, every day.
            </p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "1rem", marginBottom: "2rem" }}>
            {[
              { h: "Fandom-native audience",    b: "Her followers don't just watch — they defend, share, demand, and buy. This is the core romance fandom, not casual scrollers." },
              { h: "Algorithmic overperformer", b: "TikTok: 397.9K views in 28 days from 14K followers. YouTube Shorts peaking at 768K. Reach dramatically outpaces follower count." },
              { h: "Trusted, honest voice",     b: "She calls out bad tropes and publishing practices by name. That honesty is exactly why her audience trusts what she recommends." },
              { h: "Multi-format, daily output",b: "Daily Shorts, daily Reels, weekly essays, weekly lives, weekly newsletters. One deal, 5+ organic touchpoints." },
            ].map(v => (
              <div key={v.h} style={{ background: "#fff", borderRadius: "12px", padding: "1.25rem", border: `0.5px solid ${ROSE}22` }}>
                <div style={{ color: GOLD, fontSize: "1.1rem", marginBottom: "0.45rem" }}>✦</div>
                <div style={{ fontWeight: 600, color: WINE, fontSize: "0.84rem", marginBottom: "0.4rem" }}>{v.h}</div>
                <div style={{ fontSize: "0.77rem", color: "#666", lineHeight: 1.78 }}>{v.b}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "2rem" }}>
            {["Romance novels","Publishing industry critique","Trope analysis","Fandom culture","Author life","Reader identity","Book releases","AO3 + fanfic culture","Writing process","Industry exposé"].map(t => (
      ...
      {tab === "Stats" && (
        <div style={{ padding: "2.5rem" }}>
          ...
          <div style={{ marginBottom: "2rem" }}>
            ...
            {!yt.live && (
              <button onClick={() => setShowYT(true)}
                style={{ marginTop: "0.6rem", background: "none", border: `1px solid ${WINE}45`, color: WINE, padding: "0.22rem 0.7rem", borderRadius: "20px", cursor: "pointer", fontSize: "0.62rem", fontFamily: "inherit" }}>
                ⚡ Connect live subscriber count
              </button>
            )}
          </div>

          <div style={{ marginBottom: "2rem" }}>
            <SectionHead title="INSTAGRAM" url="https://www.instagram.com/theauthorrach/" note={ig.live ? `✦ live · ${lastIgPull}` : `updated ${lastUpdated}`} color={ROSE} />
            <StatsGrid numColor={ROSE} items={[
              { val: fmt(ig.followers), label: "Followers",       live: ig.live },
              { val: fmt(ig.posts),     label: "Posts published" },
              { val: "Daily",           label: "Reels + Stories" },
              { val: "US · UK · IN · CA · DE", label: "Top markets" },
            ]} />
            <button onClick={pullIG} disabled={igLoading}
              style={{ marginTop: "0.6rem", background: "none", border: `1px solid ${ROSE}50`, color: ROSE, padding: "0.22rem 0.7rem", borderRadius: "20px", cursor: igLoading ? "not-allowed" : "pointer", fontSize: "0.62rem", fontFamily: "inherit" }}>
              {igLoading ? "Pulling…" : "↻ Refresh Instagram live"}
            </button>
          </div>
      ...
      {tab === "Partnerships" && (
        <div style={{ padding: "2.5rem" }}>
          ...
          <p style={{ fontSize: "0.76rem", color: "#888", lineHeight: 1.75, marginBottom: "1.1rem", fontStyle: "italic", maxWidth: "540px" }}>
            Rachel's audience trust her because she has standards. Knowing you're a fit before reaching out saves everyone time — and signals we're starting from the right place.
          </p>
          <div style={{ background: CHARCOAL, borderRadius: "14px", padding: "1.75rem", marginBottom: "2.5rem" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
              <div>
                ...
                {["Book publishers + indie presses","Reading apps + e-readers","Stationery + journaling brands","Book subscription boxes","Coffee + cosy lifestyle brands","Writing tools + productivity apps","Literary merch + bookish goods"].map(b => (
              ...
              <div>
                ...
                {["Brands misaligned with feminist values","Content that erodes reader trust","Conflicts with publishing critique stance","Ethically compromised supply chains"].map(b => (
                  <div key={b} style={{ fontSize: "0.77rem", color: IVORY, opacity: 0.75, marginBottom: "0.4rem", lineHeight: 1.55 }}>{b}</div>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: WINE, borderRadius: "14px", padding: "2.5rem", textAlign: "center" }}>
            ...
            <p style={{ color: IVORY, opacity: 0.55, fontSize: "0.82rem", lineHeight: 1.8, maxWidth: "340px", margin: "0 auto 1.75rem" }}>
              All enquiries welcome. If you think you're a fit — or even if you're not sure — email me and we'll figure it out.
            </p>
